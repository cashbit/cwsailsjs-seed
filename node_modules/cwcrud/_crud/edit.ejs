<div class="container">
	<h2 class="form-signin-heading">Editing a <%= prettyName %></h2>
	<form action="../updateAction/<%= record.id %>" method="POST" role="form" class="">
					...
					<% for (var control in row) { %>
						...
						<div class="col-md-<%= width %>">
							...
							<% if (field.type == 'select') { %>
								<label class="control-label" for="<%= field.name %>"><%= field.label ? field.label : field.name %></label>
								<div class="bfh-selectbox hidden" data-name="<%= field.name %>" data-value="<%= record[field.name] ? record[field.name] : "" %>" data-filter="true">
							...
							<fieldset disabled>
								<label class="control-label" for="<%= field.name %>"><%= field.label ? field.label : field.name %></label>
								<div class="bfh-selectbox hidden disabled" data-name="<%= field.name %>" data-value="<%= record[field.name] ? record[field.name] : "" %>" data-filter="true">
									<div class="bfh-selectbox-option" data-value=""></div>
									<% _.each(field.options, function(option) { %>
										<div class="bfh-selectbox-option" data-value="<%= option.id ? option.id : option %>"><%= option.id ? option.text : option %></div>
									<% }) %>
